"""
Snakemake pipeline for cellularization dynamics.

This workflow reads raw movies from sample paths configured in `config/*.yaml`
and writes all derived outputs under `results/`,
mirroring each sample path relative to repository `data/`.

Usage:
    snakemake --cores N --configfile config/<species>.yaml
"""
...
REPO_ROOT = os.path.dirname(os.path.abspath(workflow.snakefile))
...
CELL_DYN_ROOT = os.path.join(REPO_ROOT, "results")
...
if isinstance(samples_config_arg, str) and samples_config_arg.strip():
    ...
    samples_config, samples_config_sources = load_samples_config_with_sources(
        samples_config_path
    )
...
rule all:
    input:
        ...
        expand(CELL_DYN_ROOT + "/{sample}/track/Kymograph.tif", sample=SAMPLE_RELS),
        expand(CELL_DYN_ROOT + "/{sample}/track/Kymograph_straightened.tif", sample=SAMPLE_RELS),
        expand(CELL_DYN_ROOT + "/{sample}/track/straighten_metadata.yaml", sample=SAMPLE_RELS),
        expand(CELL_DYN_ROOT + "/{sample}/track/YolkMask.tif", sample=SAMPLE_RELS),
        expand(CELL_DYN_ROOT + "/{sample}/track/VerticalKymoCelluSelection.tsv", sample=SAMPLE_RELS),
        expand(CELL_DYN_ROOT + "/{sample}/track/VerticalKymoCelluSelection.roi", sample=SAMPLE_RELS),
        expand(CELL_DYN_ROOT + "/{sample}/track/VerticalKymoCelluSelection_spline.tsv", sample=SAMPLE_RELS),
        expand(CELL_DYN_ROOT + "/{sample}/track/geometry_timeseries.csv", sample=SAMPLE_RELS)
...
rule annotate_cellu_front:
    input:
        ...
        straight=ancient(CELL_DYN_ROOT + "/{sample}/track/Kymograph_straightened.tif"),
        meta=ancient(CELL_DYN_ROOT + "/{sample}/track/straighten_metadata.yaml"),
    ...
    shell:
        """
        mkdir -p $(dirname {log:q})
        python -u scripts/cellu_front_annotation.py \
            --work-dir {params.work_dir:q} \
            > {log:q} 2>&1
        """
...
rule detect_cytoplasm_region:
    ...
    shell:
        """
        mkdir -p $(dirname {log:q})
        python -u scripts/cellu_threshold.py \
            --work-dir {params.work_dir:q} \
            > {log:q} 2>&1
        """


rule straighten_kymograph:
    input:
        kymograph=CELL_DYN_ROOT + "/{sample}/track/Kymograph.tif",
        mask=rules.detect_cytoplasm_region.output.mask,
        config=ancient(rules.init_config.output.config)
    output:
        straight=CELL_DYN_ROOT + "/{sample}/track/Kymograph_straightened.tif",
        meta=CELL_DYN_ROOT + "/{sample}/track/straighten_metadata.yaml"
    params:
        work_dir=lambda wildcards: _work_dir(wildcards.sample)
    conda:
        "envs/analysis.yaml"
    log:
        CELL_DYN_ROOT + "/{sample}/logs/straighten_kymograph.log"
    shell:
        """
        mkdir -p $(dirname {log:q})
        python scripts/straighten_kymograph.py \
            --work-dir {params.work_dir:q} \
            > {log:q} 2>&1
        """
